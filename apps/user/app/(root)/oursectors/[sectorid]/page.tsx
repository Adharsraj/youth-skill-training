...
const ElectronicsPage = ({ params }) => {
  ...
  const nextSlide = () => {
    setCurrentSlide((prevSlide) =>
      prevSlide === sector.slider.length - 1 ? 0 : prevSlide + 1
    );
  };

  const prevSlide = () => {
    setCurrentSlide((prevSlide) =>
      prevSlide === 0 ? sector.slider.length - 1 : prevSlide - 1
    );
  };
              ...
              <div className="flex flex-col md:flex-row items-center h-auto md:h-[380px] bg-primary-blue w-full px-0">
                    ...
                    <CarouselContent>
                      {sector.slider &&
                        sector.slider.map((slide, slideIndex) => (
                          <CarouselItem key={slideIndex} className="flex items-center justify-center h-[200px] md:h-[250px]">
                ...
                <div className="relative w-full md:w-1/3 h-[200px] md:h-[280px] ml-0 md:ml-8">
                  <Image
                    src={sector.s3Img}
                    alt="Section Image"
                    className="lg:w-full lg:h-full w-[300px] h-[200px] object-cover"
                    layout="fill"
                  />
                </div>
              </div>
